...
- 메인 로직
  - 다리 생성
    ...
    - 다리를 생성한다
      - 랜덤 넘버(0 또는 1)를 생성한다
      - 랜덤 넘버를 문자열(D 또는 U)로 변환한다
  - 다리 건너기
    - 이동할 칸을 입력받는다
    - 플레이어를 이동한다
  - 결과 출력
    - 중간 결과를 출력한다
      - 이동할 수 있는 칸 여부를 O 또는 X로 표시한다
      - 위 아래 방향을 구분하며 선택하지 않은 칸은 공백 표시한다
      - 다리 시작과 끝은 ‘[ ]’로 표시하며, 다리 칸은 ‘|’로 구분한다.
    - 최종 결과를 출력한다
      - 마지막 중간 결과를 출력한다
      - 게임 성공 여부를 표시한다
      - 시도 횟수를 표시한다
...
- 예외 처리
  - (다리 길이 입력값) 아래의 경우 예외처리  + 에러문구 + 재입력
    - 포맷에 맞지 않는 경우
    - 정수 타입x
    - 3이상 20이하 숫자
  - (이동할 칸 입력값) 아래의 경우 예외처리 + 에러문구 + 재입력
    - 포맷에 맞지 않는 경우
  - (재시도 명령 입력값) 아래의 경우 예외처리 + 에러문구 + 재입력
    - 포맷에 맞지 않는 경우
